/**
 * Postinstall script: patches node-pty after install.
 *
 * macOS: the spawn-helper binary inside node-pty's published prebuilds ships
 * without its executable bit. When the prebuild is used (no source rebuild),
 * every pty.spawn() fails with "posix_spawnp failed". Restore +x.
 *
 * Windows: patches node-pty's gyp files so it can build without
 * Spectre-mitigated MSVC libraries and without relying on bat scripts that
 * fail under gyp's working directory:
...
if (process.platform === 'darwin') {
  ...
  for (const arch of ['darwin-arm64', 'darwin-x64']) {
    ...
    if (fs.existsSync(helper)) {
      ...
      console.log(`[fix-node-pty] chmod +x ${arch}/spawn-helper`);
    }
  }
}
...
const genVersionH = `// AUTO-GENERATED by fix-node-pty.js
...
console.log('[fix-node-pty] Created gen/GenVersion.h');

// --- 2. Patch gyp files ---
function patchFile(filePath, patches) {
  if (!fs.existsSync(filePath)) {
    console.log(`[fix-node-pty] ${path.basename(filePath)} not found — skipping`);
    return;
  }
  ...
  if (content !== original) {
    ...
    console.log(`[fix-node-pty] Patched ${path.basename(filePath)}`);
  } else {
    console.log(`[fix-node-pty] ${path.basename(filePath)} already patched`);
  }
}
